...
def load_config():
    if os.path.isfile(CONFIG_FILE):
        ...
        return {
            ...
            "Normal": {"path": r"C:\Program Files (x86)\Steam\steamapps\common\ELDEN RING\Game\eldenring.exe", "cwd": r"C:\Program Files (x86)\Steam\steamapps\common\ELDEN RING\Game"},
...
def create_category(frame, config, key, deletable=False):
    ...
    ttk.Button(frame, text=key, command=lambda k=key: run_executable(config, k)).grid(row=row, column=0, padx=10, pady=(20, 10))
    ttk.Button(frame, text="...", command=lambda k=key, pv=path_var, cv=cwd_var: select_executable(config, k, pv, cv)).grid(row=row, column=1, padx=10, pady=(20, 10))
    ttk.Entry(frame, textvariable=path_var, width=80).grid(row=row, column=2, padx=10, pady=(20, 10))
    ttk.Entry(frame, textvariable=cwd_var, width=80).grid(row=row+1, column=2, padx=10, pady=(10, 20))
    
    if deletable:
        ttk.Button(frame, text="🗑️", command=lambda k=key: remove_category(frame, config, k)).grid(row=row, column=3, padx=10, pady=(20, 10))
...
def create_gui():
    ...
    ttk.Button(root, text="Add more", command=lambda: add_category(config, frame)).pack(pady=20)
